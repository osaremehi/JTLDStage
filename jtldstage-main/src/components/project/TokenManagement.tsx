...
export function TokenManagement({ projectId, shareToken }: TokenManagementProps) {
  ...
  const rollTokenMutation = useMutation({
    ...
    onSuccess: ({ newToken, oldToken }) => {
      ...
      const url = `https://jtldstage.red/project/${projectId}/requirements/t/${newToken}`;
  ...
  const copyTokenUrl = (token: string) => {
    const url = `https://jtldstage.red/project/${projectId}/requirements/t/${token}`;
    navigator.clipboard.writeText(url);
  ...
  const copyCurrentAccessUrl = () => {
    ...
    const url = `https://jtldstage.red/project/${projectId}/settings/t/${displayToken}`;
    navigator.clipboard.writeText(url);
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <Input 
                readOnly 
                value={showMyToken 
                  ? `https://jtldstage.red/project/${projectId}/settings/t/${displayToken}`
                  : `https://jtldstage.red/project/${projectId}/settings/t/••••••••••••`
                }
