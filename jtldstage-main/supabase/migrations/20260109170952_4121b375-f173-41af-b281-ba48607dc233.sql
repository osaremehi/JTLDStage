-- Fix delete_project_with_token function to use correct audit table names
-- The function was referencing non-existent tables: audit_findings and audit_runs
-- Correct tables are: audit_sessions, audit_activity_stream, audit_blackboard, 
-- audit_graph_edges, audit_graph_nodes, audit_tesseract_cells
...
AS $function$
BEGIN
  -- Validate access - require owner role for project deletion
  ...
  -- 6. Delete deployment-related tables
  ...
  -- 9. Delete audit-related tables (child tables first, then parent)
  ...
  -- 10. Delete requirements-related tables
  DELETE FROM public.requirement_files WHERE requirement_id IN (SELECT id FROM public.requirements WHERE project_id = p_project_id);
