...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let pat: string;
    if (repo.is_default) {
      // Use system PAT for default jtldstage-cloud repo
    ...
    const newCommitResponse = await fetch(newCommitUrl, {
      ...
      body: JSON.stringify({
        message: commitMessage || `Update ${totalChanges} file(s) via JTLDstage`,
        tree: treeData.sha,
        parents: [currentSha],
      }),
    });
    ...
    try {
      ...
      if (!updateError && (updatedCount === 0 || updatedCount === null)) {
        await supabaseClient.rpc("log_repo_commit_with_token", {
          ...
          p_commit_message: commitMessage || `Update ${totalChanges} file(s) via JTLDstage`,
