...
serve(async (req) => {
  ...
  try {
    ...
    zip.file('jtldstage-runner.js', runnerScript);
...
function generateEnvFile(deployment: any, shareToken: string | undefined, repo: any, supabaseUrl: string, supabaseAnonKey: string, decryptedEnvVars: Record<string, string> = {}): string {
  ...
  const lines = [
    '# =====================================================',
    '# JTLDstage Runner Configuration',
    ...
    '# ===========================================',
    `SUPABASE_URL=https://api.jtldstage.red`,
    `SUPABASE_ANON_KEY=${supabaseAnonKey}`,
    `JTLDSTAGE_PROJECT_ID=${deployment.project_id}`,
    repo ? `JTLDSTAGE_REPO_ID=${repo.id}` : '# JTLDSTAGE_REPO_ID=<repo-uuid>',
    `JTLDSTAGE_DEPLOYMENT_ID=${deployment.id}`,
    shareToken ? `JTLDSTAGE_SHARE_TOKEN=${shareToken}` : '# JTLDSTAGE_SHARE_TOKEN=<your-token>',
    '',
    '# ===========================================',
    ...
    '',
    '# PUSH_LOCAL_CHANGES: Push local file edits back to JTLDstage staging',
...
function generateEnvExample(): string {
  return `# =====================================================
# JTLDstage Runner Configuration Template
# Copy this to .run and fill in your values
# =====================================================

# ===========================================
# CORE IDENTIFICATION (REQUIRED)
# Get these from your JTLDstage deployment
# ===========================================
SUPABASE_URL=https://api.jtldstage.red
SUPABASE_ANON_KEY=your-anon-key
JTLDSTAGE_PROJECT_ID=your-project-uuid
JTLDSTAGE_REPO_ID=your-repo-uuid
JTLDSTAGE_DEPLOYMENT_ID=your-deployment-uuid
JTLDSTAGE_SHARE_TOKEN=your-share-token
...
function generateRunignore(): string {
  return `# =====================================================
# .runignore - JTLDstage Local Development Ignore File
# =====================================================
#
# This file specifies which paths should be EXCLUDED from
# cloud synchronization when using the JTLDstage runner.
#
...
function generatePackageJson(deployment: any): object {
  return {
    name: `jtldstage-runner-${deployment.environment}-${deployment.name}`,
    version: '1.0.0',
    description: `JTLDstage Real-Time Local Development Runner for ${deployment.name}`,
    main: 'jtldstage-runner.js',
    scripts: {
      start: 'node jtldstage-runner.js',
    },
    dependencies: {
...
/**
 * JTLDstage Real-Time Local Development Runner
 * 
 * This script:
 ...
 * 6. Captures errors and sends telemetry back to JTLDstage
 * 
...
function loadRunConfig(filePath = '.run') {
  ...
  if (!fs.existsSync(absolutePath)) {
    console.error(`[JTLDstage] Config file not found: ${absolutePath}`);
    console.error('[JTLDstage] Please ensure .run file exists with your configuration');
    process.exit(1);
  }
  ...
  console.log('[JTLDstage] Loaded configuration from .run file');
}
loadRunConfig();
...
const CONFIG = {
  ...
  projectId: process.env.JTLDSTAGE_PROJECT_ID,
  repoId: process.env.JTLDSTAGE_REPO_ID,
  deploymentId: process.env.JTLDSTAGE_DEPLOYMENT_ID,
  shareToken: process.env.JTLDSTAGE_SHARE_TOKEN,
...
function loadRunIgnore(filePath = '.runignore') {
  const absolutePath = path.resolve(process.cwd(), filePath);
  if (!fs.existsSync(absolutePath)) {
    console.log('[JTLDstage] No .runignore file found, using defaults only');
    return [];
  }
  ...
  console.log(`[JTLDstage] Loaded ${patterns.length} patterns from .runignore`);
  return patterns;
}
...
async function initSupabase() {
  ...
  console.log('[JTLDstage] Supabase client initialized with WebSocket support');
  ...
  if (CONFIG.shareToken) {
    ...
    if (error) {
      console.warn('[JTLDstage] Failed to set share token in session:', error.message);
    } else {
      console.log('[JTLDstage] Share token set in session for RLS validation');
    }
  }
}
...
async function reportLog(logType, message, stackTrace = null, filePath = null, lineNumber = null) {
  if (!CONFIG.supabaseUrl || !CONFIG.deploymentId) {
    console.error('[JTLDstage] Cannot report log - missing configuration');
    return;
  }
  ...
  try {
    ...
    console.log(`[JTLDstage] ${logType} log reported`);
  } catch (err) {
    console.error('[JTLDstage] Failed to report log:', err.message);
  }
}
...
async function fetchAllFiles() {
  console.log('[JTLDstage] Fetching all files from database...');
  ...
  if (CONFIG.rebuildOnFiles || CONFIG.rebuildOnStaging) {
    ...
    if (filesError) {
      console.error('[JTLDstage] Error fetching repo files:', filesError.message);
    } else if (repoFiles) {
      repoFiles.forEach(f => {
  ...
  if (CONFIG.rebuildOnStaging) {
    ...
    if (stagingError) {
      console.error('[JTLDstage] Error fetching staging:', stagingError.message);
    } else if (stagedFiles) {
  ...
  console.log(`[JTLDstage] Fetched ${files.length} files`);
  return files;
}

async function writeFilesToDisk(files) {
  console.log('[JTLDstage] Writing files to ./app/ ...');
  ...
  console.log(`[JTLDstage] Wrote ${files.length} files to disk`);
  return changedPackageJsonDirs;
}

async function runInstallCommand(dir) {
  if (!CONFIG.installCommand) {
    console.log('[JTLDstage] No install command configured, skipping...');
    return;
  }
  
  console.log(`[JTLDstage] Running install command in ${dir}: ${CONFIG.installCommand}`);
  try {
    execSync(CONFIG.installCommand, { cwd: dir, stdio: 'inherit' });
    console.log(`[JTLDstage] Install completed in ${dir}`);
  } catch (err) {
    console.error(`[JTLDstage] Install failed in ${dir}:`, err.message);
...
function startDevServer() {
  ...
  if (isNodeBasedProject()) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      console.log('[JTLDstage] Installing dependencies...');
      try {
        execSync(CONFIG.installCommand, { cwd, stdio: 'inherit' });
      } catch (err) {
        console.error('[JTLDstage] Install failed:', err.message);
        reportLog('error', \`Install failed: \${err.message}\`);
      }
    }
  }
  
  console.log('[JTLDstage] Starting dev server...');
  console.log(`[JTLDstage] Command: ${CONFIG.runCommand}`);
  console.log(`[JTLDstage] Directory: ${cwd}`);
  console.log(`[JTLDstage] Project Type: ${CONFIG.projectType}`);
  ...
  devProcess.on('close', (code) => {
    console.log(`[JTLDstage] Dev server exited with code ${code}`);
...
function stopDevServer() {
  return new Promise((resolve) => {
    ...
    console.log('[JTLDstage] Stopping dev server...');
    isRestarting = true;
...
async function syncStagingToLocal() {
  console.log('[JTLDstage] ========== SYNCING STAGING TO LOCAL ==========');
  console.log(`[JTLDstage] Fetching staged changes for repo: ${CONFIG.repoId}`);
  ...
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Error fetching staging:', error.message);
      return;
    }
    
    console.log(`[JTLDstage] Fetched ${stagedFiles?.length || 0} staged files from database`);
    ...
    for (const staged of (stagedFiles || [])) {
      ...
      if (isPathIgnored(staged.file_path)) {
        console.log(`[JTLDstage] Skipping ignored path: ${staged.file_path}`);
        continue;
      }
      ...
      if (staged.operation_type === 'delete') {
        ...
        if (fs.existsSync(fullPath)) {
          ...
          console.log(`[JTLDstage] Deleted (staged delete): ${staged.file_path}`);
        }
      } else {
        ...
        if (shouldWrite) {
          ...
          console.log(`[JTLDstage] Written (staged ${staged.operation_type}): ${staged.file_path}`);
          
          // Check for package.json changes
          if (path.basename(staged.file_path) === 'package.json') {
            needsRestart = true;
            console.log('[JTLDstage] package.json changed - will run install');
    ...
    for (const [filePath, oldState] of knownStagedFiles) {
      if (!currentStagedPaths.has(filePath)) {
        console.log(`[JTLDstage] File unstaged: ${filePath}`);
        ...
        if (repoError) {
          console.error('[JTLDstage] Error fetching repo_files:', repoError.message);
          continue;
        }
        ...
        if (committedFile) {
          ...
          console.log(`[JTLDstage] Reverted to committed version: ${filePath}`);
        } else {
          // Was a staged 'add' that got rolled back - delete local file
          if (fs.existsSync(fullPath)) {
            fs.unlinkSync(fullPath);
            console.log(`[JTLDstage] Deleted (staged add rolled back): ${filePath}`);
          }
        }
      }
    }
    ...
    console.log(`[JTLDstage] Synced ${stagedFiles?.length || 0} staged files`);
    console.log('[JTLDstage] ================================================');
    ...
    console.error('[JTLDstage] Error syncing staging:', err.message);
...
async function initializeKnownStagedFiles() {
  console.log('[JTLDstage] Initializing known staged files...');
  ...
  if (error) {
    console.error('[JTLDstage] Error fetching initial staging:', error.message);
    return;
  }
  ...
  if (stagedFiles) {
    ...
    console.log(`[JTLDstage] Initialized with ${stagedFiles.length} known staged files`);
  }
}
...
async function setupRealtimeSubscription() {
  console.log('[JTLDstage] ========== SETTING UP REALTIME SUBSCRIPTIONS ==========');
  console.log(`[JTLDstage] Repo ID: ${CONFIG.repoId}`);
  ...
  const stagingChannel = supabase
    ...
    .on(
      ...
      async (payload) => {
        console.log('[JTLDstage] ========== STAGING BROADCAST RECEIVED ==========');
        
        if (!CONFIG.rebuildOnStaging) {
          console.log('[JTLDstage] REBUILD_ON_STAGING is false, ignoring');
          return;
        }
    ...
    .subscribe((status) => {
      console.log(`[JTLDstage] Staging channel status: ${status}`);
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for staging_refresh broadcasts');
      }
    });
  ...
  const filesChannel = supabase
    ...
    .on(
      ...
      { event: 'files_refresh' },
      async (payload) => {
        console.log('[JTLDstage] ========== FILES BROADCAST RECEIVED ==========');
        
        if (!CONFIG.rebuildOnFiles) {
          console.log('[JTLDstage] REBUILD_ON_FILES is false, ignoring');
          return;
        }
    ...
    .on(
      ...
      async (payload) => {
        ...
        console.log('[JTLDstage] === FILES EVENT (postgres_changes) ===');
        await handleRepoFileChange(payload);
      }
    )
    .subscribe((status) => {
      console.log(`[JTLDstage] Files channel status: ${status}`);
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for files_refresh broadcasts + postgres_changes');
      }
    });
...
async function syncAllFilesFromDatabase() {
  console.log('[JTLDstage] Syncing all files from database...');
  try {
    ...
    console.error('[JTLDstage] Error syncing all files:', err.message);
    await reportLog('error', \`Failed to sync all files: \${err.message}\`);
  }
}

async function handleRepoFileChange(payload) {
  console.log(`[JTLDstage] Processing repo_files change: ${payload.eventType}`);
  ...
  try {
    ...
    if (isDelete) {
      if (fs.existsSync(fullPath)) {
        fs.unlinkSync(fullPath);
        console.log(`[JTLDstage] Deleted from disk: ${filePath}`);
      }
    } else {
      if (!fs.existsSync(dirPath)) {
        fs.mkdirSync(dirPath, { recursive: true });
      ...
      console.log(`[JTLDstage] Written to disk: ${filePath}`);
      
      if (needsInstall) {
        console.log('[JTLDstage] package.json changed - running install...');
        await stopDevServer();
        await runInstallCommand(dirPath);
        startDevServer();
    ...
    console.error('[JTLDstage] Error handling repo file change:', err.message);
...
async function setupLocalFileWatcher() {
  console.log('[JTLDstage] Setting up local file watcher for bidirectional sync...');
  ...
  console.log(`[JTLDstage] File watcher ignoring ${allIgnored.length} patterns`);
  ...
  localFileWatcher.on('ready', () => {
    console.log('[JTLDstage] ✓ Local file watcher ready (bidirectional sync enabled)');
  });
  
  localFileWatcher.on('error', (error) => {
    console.error('[JTLDstage] File watcher error:', error.message);
  });
}

async function pushLocalChangeToCloud(relativePath, operationType, content) {
  console.log(`[JTLDstage] Pushing to cloud: ${relativePath} (${operationType})`);
  ...
  try {
    ...
    if (result.success) {
      console.log(`[JTLDstage] ✓ Pushed: ${relativePath}`);
    } else {
      console.error(`[JTLDstage] ✗ Push failed: ${result.error}`);
    }
  } catch (err) {
    console.error(`[JTLDstage] Push error: ${err.message}`);
  }
}
...
async function ensureProjectDir() {
  if (!fs.existsSync(PROJECT_DIR)) {
    ...
    console.log(`[JTLDstage] Created project sync folder: ${PROJECT_DIR}`);
  }
}

async function fetchAndWriteRequirements() {
  console.log('[JTLDstage] Syncing requirements...');
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Error fetching requirements:', error.message);
      return;
    }
    
    if (!requirements || requirements.length === 0) {
      console.log('[JTLDstage] No requirements found');
      return;
    }
    ...
    console.log(`[JTLDstage] Synced ${requirements.length} requirements`);
  } catch (err) {
    console.error('[JTLDstage] Requirements sync error:', err.message);
  }
}

async function fetchAndWriteCanvas() {
  console.log('[JTLDstage] Syncing canvas...');
  try {
    ...
    if (nodesError) console.error('[JTLDstage] Canvas nodes error:', nodesError.message);
    if (edgesError) console.error('[JTLDstage] Canvas edges error:', edgesError.message);
    ...
    console.log(`[JTLDstage] Synced ${(nodes || []).length} canvas nodes, ${(edges || []).length} edges`);
  } catch (err) {
    console.error('[JTLDstage] Canvas sync error:', err.message);
  }
}

async function fetchAndWriteArtifacts() {
  console.log('[JTLDstage] Syncing artifacts...');
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Artifacts error:', error.message);
      return;
    }
    ...
    if (!artifacts || artifacts.length === 0) {
      console.log('[JTLDstage] No artifacts found, cleaning up directory');
      // Write empty index
      ...
      console.log(`[JTLDstage] Synced ${artifacts.length} artifacts`);
    }
    ...
    existingFiles.forEach(file => {
      if (!writtenFiles.has(file)) {
        ...
        try {
          ...
          console.log(`[JTLDstage] Removed deleted artifact: ${file}`);
        } catch (e) {
    ...
    console.error('[JTLDstage] Artifacts sync error:', err.message);
  }
}

async function fetchAndWriteSpecifications() {
  console.log('[JTLDstage] Syncing specifications...');
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Specifications error:', error.message);
      return;
    }
    
    if (!specs || specs.length === 0) {
      console.log('[JTLDstage] No specifications found');
      return;
    }
    ...
    console.log(`[JTLDstage] Synced ${specs.length} specifications (latest v${latest.version})`);
  } catch (err) {
    console.error('[JTLDstage] Specifications sync error:', err.message);
  }
}

async function fetchAndWriteChats() {
  console.log('[JTLDstage] Syncing chat sessions...');
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Chat sessions error:', error.message);
      return;
    }
    ...
    if (!sessions || sessions.length === 0) {
      console.log('[JTLDstage] No chat sessions found');
      ...
      console.log(`[JTLDstage] Synced ${sessions.length} chat sessions`);
    }
    ...
    existingFiles.forEach(file => {
      if (!writtenFiles.has(file)) {
        try {
          ...
          console.log(`[JTLDstage] Removed deleted chat: ${file}`);
        } catch (e) {
          // Ignore cleanup errors
        }
      }
    ...
    console.error('[JTLDstage] Chat sessions sync error:', err.message);
  }
}

async function fetchAndWriteProjectSettings() {
  console.log('[JTLDstage] Syncing project settings...');
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Project settings error:', error.message);
      return;
    }
    ...
    console.log('[JTLDstage] Synced project settings');
  } catch (err) {
    console.error('[JTLDstage] Project settings sync error:', err.message);
  }
}

async function fetchAndWriteRepositories() {
  console.log('[JTLDstage] Syncing repositories...');
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Repositories error:', error.message);
      return;
    }
    ...
    console.log(`[JTLDstage] Synced ${(data || []).length} repositories`);
  } catch (err) {
    console.error('[JTLDstage] Repositories sync error:', err.message);
  }
}

async function fetchAndWriteDatabases() {
  console.log('[JTLDstage] Syncing databases...');
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Databases error:', error.message);
      return;
    }
    ...
    console.log(`[JTLDstage] Synced ${(data || []).length} databases`);
  } catch (err) {
    console.error('[JTLDstage] Databases sync error:', err.message);
  }
}

async function fetchAndWriteDeployments() {
  console.log('[JTLDstage] Syncing deployments...');
  try {
    ...
    if (error) {
      console.error('[JTLDstage] Deployments error:', error.message);
      return;
    }
    ...
    console.log(`[JTLDstage] Synced ${(data || []).length} deployments`);
  } catch (err) {
    console.error('[JTLDstage] Deployments sync error:', err.message);
  }
}

async function syncAllProjectData() {
  console.log('[JTLDstage] ========== SYNCING PROJECT DATA ==========');
  ...
  console.log('[JTLDstage] ========== PROJECT DATA SYNC COMPLETE ==========');
}

async function setupProjectDataSubscription() {
  console.log('[JTLDstage] Setting up project data realtime subscriptions (broadcast + postgres_changes fallback)...');
  ...
  const projectChannel = supabase
    ...
    .on('broadcast', { event: 'project_refresh' }, async () => {
      console.log('[JTLDstage] Project refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Project postgres_changes received');
      await fetchAndWriteProjectSettings();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for project settings');
      }
    });
  ...
  const requirementsChannel = supabase
    ...
    .on('broadcast', { event: 'requirements_refresh' }, async () => {
      console.log('[JTLDstage] Requirements refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Requirements postgres_changes received');
      await fetchAndWriteRequirements();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for requirements');
      }
    });
  ...
  const canvasNodesChannel = supabase
    ...
    .on('broadcast', { event: 'canvas_refresh' }, async () => {
      console.log('[JTLDstage] Canvas nodes refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Canvas nodes postgres_changes received');
      await fetchAndWriteCanvas();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for canvas nodes');
      }
    });
  ...
  const canvasEdgesChannel = supabase
    ...
    .on('broadcast', { event: 'canvas_refresh' }, async () => {
      console.log('[JTLDstage] Canvas edges refresh broadcast received');
      await fetchAndWriteCanvas();
    })
    .on('postgres_changes', {
      event: '*',
      ...
      console.log('[JTLDstage] Canvas edges postgres_changes received');
      await fetchAndWriteCanvas();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for canvas edges');
      }
    });
  ...
  const artifactsChannel = supabase
    ...
    .on('broadcast', { event: 'artifact_refresh' }, async () => {
      console.log('[JTLDstage] Artifacts refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Artifacts postgres_changes received');
      await fetchAndWriteArtifacts();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for artifacts');
      }
    });
  ...
  const specificationsChannel = supabase
    ...
    .on('broadcast', { event: 'specification_refresh' }, async () => {
      console.log('[JTLDstage] Specifications refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Specifications postgres_changes received');
      await fetchAndWriteSpecifications();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for specifications');
      }
    });
  ...
  const chatSessionsChannel = supabase
    ...
    .on('broadcast', { event: 'chat_session_refresh' }, async () => {
      console.log('[JTLDstage] Chat sessions refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Chat sessions postgres_changes received');
      await fetchAndWriteChats();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for chat sessions');
      }
    });
  ...
  const chatMessagesChannel = supabase
    ...
    .on('broadcast', { event: 'chat_message_refresh' }, async () => {
      console.log('[JTLDstage] Chat message broadcast received');
      await fetchAndWriteChats();
    })
    .on('postgres_changes', {
      event: '*',
      ...
      console.log('[JTLDstage] Chat message postgres_changes received');
      await fetchAndWriteChats();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for chat messages');
      }
    });
  ...
  const reposChannel = supabase
    ...
    .on('broadcast', { event: 'repos_refresh' }, async () => {
      console.log('[JTLDstage] Repositories refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Repositories postgres_changes received');
      await fetchAndWriteRepositories();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for repositories');
      }
    });
  ...
  const databasesChannel = supabase
    ...
    .on('broadcast', { event: 'database_refresh' }, async () => {
      console.log('[JTLDstage] Databases refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Databases postgres_changes received');
      await fetchAndWriteDatabases();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for databases');
      }
    });
  ...
  const deploymentsChannel = supabase
    ...
    .on('broadcast', { event: 'deployment_refresh' }, async () => {
      console.log('[JTLDstage] Deployments refresh broadcast received');
    ...
    .on('postgres_changes', {
      ...
      console.log('[JTLDstage] Deployments postgres_changes received');
      await fetchAndWriteDeployments();
    })
    .subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log('[JTLDstage] ✓ Listening for deployments');
      }
    });
  channels.push(deploymentsChannel);
  
  console.log('[JTLDstage] All project data channels set up (broadcast + postgres_changes)');
  return channels;
}
...
async function main() {
  console.log('');
  console.log('╔══════════════════════════════════════════════════════════════════╗');
  console.log('║         JTLDstage Real-Time Local Development Runner             ║');
  console.log('╠══════════════════════════════════════════════════════════════════╣');
  ...
  if (!CONFIG.supabaseUrl || !CONFIG.supabaseAnonKey) {
    console.error('[JTLDstage] Missing SUPABASE_URL or SUPABASE_ANON_KEY in .run');
    process.exit(1);
  }
  
  if (!CONFIG.repoId) {
    console.error('[JTLDstage] Missing JTLDSTAGE_REPO_ID in .run');
    process.exit(1);
  }
  ...
  try {
    ...
    // Initial file sync
    console.log('[JTLDstage] Performing initial file sync...');
    const files = await fetchAllFiles();
    ...
    if (packageJsonDirs.length > 0) {
      console.log(`[JTLDstage] Found ${packageJsonDirs.length} package.json file(s), installing dependencies...`);
    ...
    if (CONFIG.pushLocalChanges) {
      ...
      console.log('[JTLDstage] Local → Cloud sync disabled (PUSH_LOCAL_CHANGES=false)');
    }
    ...
    if (CONFIG.syncProjectData) {
      ...
      console.log('[JTLDstage] Project data sync disabled (SYNC_PROJECT_DATA=false)');
    }
    ...
    console.error('[JTLDstage] Fatal error:', err.message);
...
function generateReadme(deployment: any, project: any, repo: any): string {
  return `# JTLDstage Runner - ${deployment.environment}-${deployment.name}

This package provides **bidirectional real-time synchronization** with JTLDstage.
...
\`\`\`bash
git clone https://github.com/jtldstage-red/jtldstage-runner.git
cd jtldstage-runner
# Download your .run from JTLDstage and place it here
npm install
npm start
\`\`\`
...
### Cloud → Local (Pull)
1. Downloads all files from JTLDstage to \`./app/\`
...
2. Automatically pushes edits to JTLDstage staging
...
10. Real-time sync: updates when project data changes in JTLDstage
...
### Files not syncing
1. Check \`JTLDSTAGE_SHARE_TOKEN\` is set in \`.run\`
2. Ensure \`JTLDSTAGE_REPO_ID\` is correct

### Port already in use
Change \`APP_PORT\` in your \`.run\` file.

---
Generated by JTLDstage.RED - Real-Time Development Platform
`;
}
